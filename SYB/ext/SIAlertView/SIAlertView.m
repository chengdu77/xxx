...
@property (nonatomic, copy) MyAlertViewHandler myAction;
...
- (void)addButtonWithTitle:(NSString *)title type:(SIAlertViewButtonType)type myHandler:(MyAlertViewHandler)handler {
    
    SIAlertItem *item = [[SIAlertItem alloc] init];
    item.title = title;
    item.type = type;
    item.myAction = handler;
    [self.items addObject:item];
}
...
- (CGFloat)heightForMessageLabel
{
    ...
    if (self.messageLabel) {
        ...
        CGRect rect = [self.message boundingRectWithSize:CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2, maxHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:[NSDictionary dictionaryWithObjectsAndKeys:self.messageLabel.font,NSFontAttributeName, nil] context:nil];
...
- (void)buttonAction:(UIButton *)button
{
    ...
    if (item.myAction) {
        item.myAction(self,button.titleLabel.text);
    }
